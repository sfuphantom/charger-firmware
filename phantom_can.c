...
{
    uint16 incoming_data[4] = { 0 }; //make it a struct
    ...
    incoming_data[0] = outputVoltage;
    incoming_data[1] = outputCurrent;
    incoming_data[2] = outputState;

    uint16 *incoming_data_ptr1 = &incoming_data[0];

    int j = 0;
    for (j = 0; j < 4; j++)
    {
        // can't find a workaround to the uint16, the DBC needs it
        canReceive(canREG1, canMESSAGE_BOX9, incoming_data_ptr);
        incoming_data_ptr += 16; /* next chunk ...*/
    }
}
